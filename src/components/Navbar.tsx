                ...
                <div className="hidden lg:flex items-center gap-8">
                    {navLinks.map((link) => (
                        <Link
                            key={link.to}
                            to={link.to}
                            className={`text-base font-extrabold transition-colors hover:text-cta ${location.pathname === link.to ? "text-cta" : "text-foreground"
                                }`}
                        >
                            {link.label}
                        </Link>
                ...
                <div className="lg:hidden border-t border-border bg-background px-4 pb-6 pt-2 shadow-lg">
                    {navLinks.map((link) => (
                        <Link
                            ...
                            className={`block py-3 text-lg font-extrabold transition-colors ${location.pathname === link.to ? "text-cta" : "text-foreground"
                                }`}
                        >
                            {link.label}
                        </Link>
